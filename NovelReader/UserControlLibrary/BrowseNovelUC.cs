...
namespace NovelReader.UserControlLibrary
{
    public partial class BrowseNovelUC : UserControl
    {
        ...
        private async Task LoadNovelDataToCardAsync(string url, bool isSearch)
        {
            ...
            List<Task<NovelCard>> novelTasks = new List<Task<NovelCard>>();
        ...
        private NovelCard AddNovelCardValue(string title, string latestchapter, string link, string imglink, string rating)
        {
            NovelCard nc = new NovelCard();
            return nc.SendNovelCardData(title, latestchapter, link, imglink, rating);
        }

        private void NovelCardToPanel(NovelCard novelCard)
        {
        ...
        {
            foreach (NovelCard novelCard in mypanel.Controls)
            {
        ...
        {
            foreach (var c in this.flowLayoutPanel1.Controls.OfType<NovelCard>())
            {
